package com.zhaofang.yushu.common.threadpool;
...
public class ThreadUtil {
    ...
    /**
     * 获得一个新的线程池<br> 传入阻塞系数，线程池的大小计算公式为：CPU可用核心数 / (1 - 阻塞因子)<br> Blocking Coefficient(阻塞系数) =
     * 阻塞时间／（阻塞时间+使用CPU的时间）<br> 计算密集型任务的阻塞系数为0，而IO密集型任务的阻塞系数则接近于1。
     *
    ...
    /**
     * 执行有返回值的异步方法<br> Future代表一个异步执行的操作，通过get()方法可以获得操作的结果，如果异步操作还没有完成，则，get()会使当前线程阻塞
     *
     * @param <T> 回调对象类型
    ...
    /**
     * 新建一个CompletionService，调用其submit方法可以异步执行多个任务，最后调用take方法按照完成的顺序获得其结果。<br> 若未完成，则会阻塞
     *
     * @param <T> 回调对象类型
    ...
    /**
     * 获取JVM中与当前线程同组的所有线程<br> 使用数组二次拷贝方式，防止在线程列表获取过程中线程终止<br> from Voovan
     *
     * @param group 线程组
     * @return 线程对象数组
     */
